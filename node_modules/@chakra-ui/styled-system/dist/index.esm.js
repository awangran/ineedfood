...
import { isObject as isObject4, runIfFn } from "@chakra-ui/utils";
var expandResponsive = (styles) => (theme) => {
  ...
  for (const key in styles) {
    ...
    if (value == null)
      continue;
    value = isObject4(value) && isResponsive(value) ? toArrayValue(value) : value;
  ...
  const css2 = (stylesOrFn, nested = false) => {
    ...
    for (let key in styles) {
      ...
      if (isObject5(value)) {
        ...
        computedStyles[key] = merge2({}, computedStyles[key], css2(value, true));
        continue;
      }
      ...
      if (!nested && (config == null ? void 0 : config.static)) {
        ...
        computedStyles = merge2({}, computedStyles, staticStyles);
      }
      ...
      if (configProperty) {
        if (configProperty === "&" && isObject5(rawValue)) {
          computedStyles = merge2({}, computedStyles, rawValue);
        } else {
          computedStyles[configProperty] = rawValue;
        }
        continue;
      }
      if (isObject5(rawValue)) {
        computedStyles = merge2({}, computedStyles, rawValue);
        continue;
      }
...
function resolveStyleConfig(config) {
  return (props) => {
    ...
    return mergeWith3({}, runIfFn3(config.baseStyle ?? {}, props), recipe(config, "sizes", size, props), recipe(config, "variants", variant, props));
  };
}
