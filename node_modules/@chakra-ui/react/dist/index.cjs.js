...
// src/extend-theme.ts
var import_theme2 = require("@chakra-ui/theme");
var import_utils = require("@chakra-ui/utils");
function extendTheme(...extensions) {
  let overrides = [...extensions];
  let baseTheme = extensions[extensions.length - 1];
  if ((0, import_theme2.isChakraTheme)(baseTheme) && overrides.length > 1) {
    overrides = overrides.slice(0, overrides.length - 1);
  } else {
    baseTheme = import_theme2.theme;
  }
  return (0, import_utils.pipe)(...overrides.map((extension) => (prevTheme) => (0, import_utils.isFunction)(extension) ? extension(prevTheme) : mergeThemeOverride(prevTheme, extension)))(baseTheme);
}
function mergeThemeOverride(...overrides) {
  return (0, import_utils.mergeWith)({}, ...overrides, mergeThemeCustomizer);
}
function mergeThemeCustomizer(source, override, key, object) {
  if (((0, import_utils.isFunction)(source) || (0, import_utils.isFunction)(override)) && Object.prototype.hasOwnProperty.call(object, key)) {
    return (...args) => {
      const sourceValue = (0, import_utils.isFunction)(source) ? source(...args) : source;
      const overrideValue = (0, import_utils.isFunction)(override) ? override(...args) : override;
      return (0, import_utils.mergeWith)({}, sourceValue, overrideValue, mergeThemeCustomizer);
    };
  }
  return void 0;
}
...
function withDefaultColorScheme({
  ...
  return (theme3) => {
    ...
    if (Array.isArray(components)) {
      names = components;
    } else if ((0, import_utils2.isObject)(components)) {
      names = Object.keys(components);
    }
    return mergeThemeOverride(theme3, {
      components: (0, import_utils2.fromEntries)(names.map((componentName) => {
...
function withDefaultSize({
  ...
  return (theme3) => {
    ...
    if (Array.isArray(components)) {
      names = components;
    } else if ((0, import_utils3.isObject)(components)) {
      names = Object.keys(components);
    }
    return mergeThemeOverride(theme3, {
      components: (0, import_utils3.fromEntries)(names.map((componentName) => {
        const withSize = {
          defaultProps: {
            size
          }
...
function withDefaultVariant({
  ...
  return (theme3) => {
    ...
    if (Array.isArray(components)) {
      names = components;
    } else if ((0, import_utils4.isObject)(components)) {
      names = Object.keys(components);
    }
    return mergeThemeOverride(theme3, {
      components: (0, import_utils4.fromEntries)(names.map((componentName) => {
        const withVariant = {
          defaultProps: {
            variant
          }
...
function withDefaultProps({
  ...
  return (theme3) => mergeThemeOverride((0, import_utils5.pipe)(...fns)(theme3));
}
